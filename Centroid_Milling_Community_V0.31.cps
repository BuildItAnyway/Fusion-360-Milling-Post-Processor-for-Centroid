...
description = "CENTROID Milling community 0.3";
vendor = "CENTROID";
...
longDescription = "CENTROID Milling community 0.31";
...
properties = {
  ...
  checkDuplicateToolNr: true, // Verifies if the same Tool Number has been used with different Geometries
  maxToolNr: 200, // Sets the maximum allowed Tool #
...
propertyDefinitions = {
  ...
  showSequenceNumbers: {title:"Use sequence numbers", description:"Use sequence numbers for each block of outputted code.", group:0, type:"boolean"},
  sequenceNumberStart: {title:"Start sequence number", description:"The number at which to start the sequence numbers.", group:0, type:"integer"},
  sequenceNumberIncrement: {title:"Sequence number increment", description:"The amount by which the sequence number is incremented by in each block.", group:0, type:"integer"},
  optionalStop: {title:"Optional stop", description:"Outputs optional stop code during when necessary in the code.", group:0, type:"boolean"},
  separateWordsWithSpace: {title:"Separate words with space", description:"Adds spaces between words if 'yes' is selected.", group:0, type:"boolean"},
  useRadius: {title:"Radius arcs", description:"If yes is selected, arcs are outputted using radius values rather than IJK.", group:0, type:"boolean"},
  //Begin Property Definitions added by -swissi
  checkToolOffset: {title: "Check Tool Offset", description:"Chose between an M0 stop or M200 message to verify Tool Height Offset after tool length comp move.", group:1, type:"enum",
    values:[
      {title:"No", id:"No"},
      {title:"M0", id:"M0"},
      {title:"M200", id:"M200"}  ] },
  writeCNC12Vars: {title: "Write CNC12 Info Variables", description:"Write CNC12 User-String-Variables with Fusion 360 Information.", group:1, type:"boolean"},
  addDebugInfo: {title:"Add Debug Information", description:"Turn post debugging info On or Off", group:1, type:"boolean"},
  safeRetract: {
    title: "Safe Retracts",
    description: "Select safe retract option. 'Clearance Height' retracts to the operation clearance height.", group:1, type: "enum",
       values:[
       {title:"G28", id:"G28"},
       {title:"G30", id:"G30"},
  ...
  homeZatEnd: {title:"Z-Positions at End of Job", description:"Selecting No Move is very risky. Only select if you know what you are doing!", group:1, type:"enum",
	  values:[
	   {title:"G28", id:"G28"},
	   {title:"G30", id:"G30"},
	   {title:"G30 P3", id:"G30P3"},
  ...
  homeXYatEnd: {title:"XY-Positions at End of Job", description:"Select Return Position of X/Y Axis at End of Job", group:1, type:"enum",
	  values:[
	   {title:"G28", id:"G28"},
	   {title:"G30", id:"G30"},
	   {title:"G30 P3", id:"G30P3"},
       {title:"G30 P4", id:"G30P4"},
   	   {title:"No Move", id:"none"}  ] },   
  dwellFactor: {title:"Dwell after Spindle Start (% Factor of rpm)", description:"Adds a Dwell command after a Spindle Start. This factor is the percentage of the spindel speed. A factor of 100 will delay 5000rpm by 5 seconds. (0 to disable)", group:1, type:"integer"},
  clampEnable: {title:"Enable Clamp On/Off (M10/M11)", description:"Enables Clamp On/Off command M10/M11", group:1, type:"boolean"},
  forceNumeric: {title:"Enforce Numeric Program Name", description:"Forces the Program Name to be numeric. Set to No to allow non-numeric Program Names", group:1, type:"boolean"},
  addCommandBegin: {title:"Add Command to Begin of Job", description:"Add a command to the begin of the job", group:1, type:"string"},
  addCommandEnd: {title:"Add Command to End of Job", description:"Add a command to the end of the job", group:1, type:"string"},
  commentFormat: {title:"Comment Line Formatting", description:"Select the formatting of the Comment line", group:1, type:"enum",
    values:[
      {title:"( )", id:"( )"},
      {title:":", id:":"}  ] },
  rotaryTableAxis: {title:"Rotary Table Axis", description:"Select the rotary table axis orientation. Along X=A, Y=B, Z=C", group:1, type:"enum",
	  values:[
	   ...
	   {title:"Along -Y", id:"-b"},
       {title:"Along -Z", id:"-c"}     ] },
  resetRotary: {title:"Reset Rotary Axis at End of Job", description:"Reset Rotary Axis at End of Job", group:1, type:"boolean"},     
  smoothingProfile: {title:"Smoothing Profile", description:"-1 to use current CNC12 settings, 0 to turn off smoothing, 1-99 to activate that smoothing profile.", group:1, type:"integer"},
  checkCNC12ToolLibrary: {title:"Check/Update CNC12 Tool Library ", description:"Compares Tool Diameter between Fusion and CNC12 Tool Library and updates CNC12 if requested", group:1, type:"boolean"},
  checkDuplicateToolNr: {title:"Check Tool# for Duplicates", description:"Checks if the same Tool# has been used with different Geometries", group:1, type:"boolean"},
  maxToolNr: {title:"Maximum allowed Number of Tools", description:"If this Tool# is exceeded, the Post Processor will fail with an Error Message", group:1, type:"integer"},
...
var comparisonTolerance = 0.000001  //01/18/2021 -swissi
...
function onOpen() {
  ...
  if (properties.checkDuplicateToolNr) { 
    for (var i = 0; i < getNumberOfSections(); ++i) { 
  ...
  if (properties.forceNumeric) {  
    ...
    if ((programId >= 9100) && (programId <= 9999)) {
		  error(localize("CNC12 does not allow Program Numbers 9100 - 9999."));
		}
  ...
  if (properties.checkCNC12ToolLibrary) {
    ...
    //writeBlock("#102 = fix[[#[11000 + #[13000 + #[" + toolFormat.format(toolStartNrVar) + " + #101]]] + 0.000001] * 100000] / 100000");
    /*writeBlock("If #102 != #[" + toolFormat.format(toolStartNrVar) + 
    ...
    */
    writeBlock("If ABS[#[11000 + #[13000 + #[" + toolFormat.format(toolStartNrVar) + " + #101]]] - #[" + toolFormat.format(toolStartNrVar) + " + #101 + 200]] > " + comparisonTolerance + 
               " Then M224 #100 \"WARNING!\\n\\nTool #\%.0f Diameter does not match!\\n\\nFusion D: \%f\\nCNC12 D: \%f\\n\\n#\)[A]bort\\n#\)[I]gnore and continue\\n#\)[U]pdate CNC12 Library\" #[" 
               + toolFormat.format(toolStartNrVar) + " + #101] #[" + toolFormat.format(toolStartNrVar + 200) + " + #101] #[11000 + #[13000 + #[" + toolFormat.format(toolStartNrVar) + " + #101]]]");
    ...
    if (forceMultiAxisIndexing || !is3D() || machineConfiguration.isMultiAxisConfiguration()) { // use 5-axis indexing for multi-axis mode
      ...
      if (_section.isMultiAxis()) {
        ...
        if (_setWorkPlane) {
          ...
          forceWorkPlane();
          if (properties.clampEnable) {onCommand(COMMAND_UNLOCK_MULTI_AXIS);}
          gMotionModal.reset();
          writeBlock(
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    //moved here by swissi 01/22/2021
    if (insertToolCall) {
  ...
  if (insertToolCall) {
    ...
    if (tool.number > properties.maxToolNr) {
    ...
    if (properties.preloadTool) {
      ...
      if (nextTool) {
        writeBlock("T" + toolFormat.format(nextTool.number) + " M107");
      } else {
        // preload first tool
        var section = getSection(0);
        var firstToolNumber = section.getTool().number;
        if (tool.number != firstToolNumber) {
          writeBlock("T" + toolFormat.format(firstToolNumber) + " M107");
        }
      }
    }
  }
  ...
  if (insertToolCall || retracted) {
    ...
    if (lengthOffset > properties.maxToolNr) {
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
      case "tapping-with-chip-breaking": // Uwe Peck Tapping // added by spikee 02-01-2025
